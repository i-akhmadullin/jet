...
jet.description = 'Init a new html5-boilerplate project.';

// Template-specific notes to be displayed before question prompts.
jet.notes = fs.readFileSync(path.join(__dirname, 'h5bp/notes.txt'), 'utf8');

// Any existing file or directory matching this wildcard will cause a warning.
jet.warnOn = '*';

// the "prompts steps"
jet.steps = 'project setup files gruntfile'.split(' ');

// The actual init template.
jet.template = function(grunt, init, done) {
  jet.grunt = grunt;
  ...
  (function run(step) {
    if(!step) return jet.end(init, jet.props, done);

    jet[step](function(err, props) {
      if(err) {
        grunt.log.error(err);
        return done(false);
      }

      if(step === 'project') jet.props = props;
      else jet.props[step] = props;
      run(jet.steps.shift());
    });
  })(jet.steps.shift())
...
// description, etc.)
jet.project = function project(cb) {
  ...
  jet.prompt(prompts.trim().split(' '), function(err, props) {
...
jet.end = function end(init, props, cb) {
  var grunt = this.grunt;

  // custom renames
  init.renames = jet.renames(init, props);
...
jet.customPrompt = function() {
  var grunt = this.grunt,
    opts = grunt.helper('prompt_for_obj'),
    custom = grunt.file.read(path.join(__dirname, 'jet/prompts.txt')).trim(),
    matcher = /^\[\?\]\s?([^\(]+)\(([^\)]+)\)\s*\|\s([\w\d\-_\/]+)/,
...
jet.prompt = function(prompts, cb) {
  var grunt = this.grunt;
...
// directory mappings.
jet.renames = function(init, props) {
