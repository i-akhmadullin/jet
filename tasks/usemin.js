...
module.exports = function(grunt) {
  ...
  grunt.registerHelper('usemin:post:html', function(content) {
    ...
    content = grunt.helper('replace:js', content, /<script.+src=['"](.+)["'][\/>]?><[\\]?\/script>/gm);
  ...
  grunt.registerHelper('replace', function(content, regexp) {
    return content.replace(regexp, function(match, src) {
      ...
      if(src.match(/\/\//)) {
        console.log( 'do not touch external files' );
        return match;
      }
      var basename = path.basename(src);
      var extname = path.extname(basename);
      basename = basename.replace(extname, '.min.css');
      var dirname = path.dirname(src);

      // XXX files won't change, the filepath should filter the original list
      // of cached files.
      var filepath = grunt.file.expand(path.join('publish/') + basename)[0];
      console.log( basename, path.join('publish/') );
      console.log( 'path.join("publish/") + basename '+path.join('publish/') + basename );
      console.log( 'filepath1 '+filepath );
      // not a file in intermediate, skip it
      if(!filepath) {
        console.log( 'not a file in intermediate, skip it' );
        return match;
      }
      ...
      if(!filename) {
        console.log( ' if file not exists probaly was concatenated into another file so skip it' );
        return '';
      }
  ...
  /* dirty fix */
  grunt.registerHelper('replace:js', function(content, regexp) {
    return content.replace(regexp, function(match, src) {
      //do not touch external files
      if(src.match(/\/\//)) {
        return match;
      ...
      var basename = path.basename(src);
      var extname = path.extname(basename);
      basename = basename.replace(extname, '.min.js');
      var dirname = path.dirname(src);
